...
- [**Ethos**](https://github.com/punt-labs/ethos) `CLI` `Claude Code` `Go` — Identity binding for humans and AI agents. One YAML file per identity with channel bindings — voice (Vox), email (Beadle), GitHub (Biff), Claude Code agent definition, writing style, personality, and skills. Sidecar architecture: other tools read identity state optionally.
...
```bash
curl -fsSL https://raw.githubusercontent.com/punt-labs/punt-kit/9aa17dd/install-all.sh | sh
```
