...
banner "punt-kit"
curl -fsSL "$GH/punt-kit/dd46b44/install.sh" | sh
...
banner "quarry"
curl -fsSL "$GH/quarry/4bffcc0/install.sh" | sh

banner "vox"
curl -fsSL "$GH/vox/25cce86/install.sh" | sh

banner "lux"
curl -fsSL "$GH/lux/ddf25c0/install.sh" | sh

banner "ethos"
curl -fsSL "$GH/ethos/fbc9a1f/install.sh" | sh
