...
banner "Marketplace"
curl -fsSL "$GH/claude-plugins/a704a54/install.sh" | sh

# --- Step 2: CLI tools ---
# Each installer handles its own pre-flight (Python, uv, SSH fallback).

banner "punt-kit"
curl -fsSL "$GH/punt-kit/278e9cc/install.sh" | sh

banner "beadle"
curl -fsSL "$GH/beadle/4cc7f6b/install.sh" | sh

banner "biff"
curl -fsSL "$GH/biff/7f2d961/install.sh" | sh

banner "quarry"
curl -fsSL "$GH/quarry/fd274d3/install.sh" | sh

banner "vox"
curl -fsSL "$GH/vox/d968a18/install.sh" | sh

banner "lux"
curl -fsSL "$GH/lux/e6ee040/install.sh" | sh

banner "ethos"
curl -fsSL "$GH/ethos/c513be7/install.sh" | sh

banner "z-spec"
curl -fsSL "$GH/z-spec/6cc8ba8/install.sh" | sh
